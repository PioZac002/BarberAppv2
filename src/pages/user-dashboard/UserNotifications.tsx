...
import { Bell, Calendar, CheckCircle, Trash2, Gift, AlertCircle, Star, Settings, ShieldCheck, HelpCircle, Info } from "lucide-react";
...
const UserNotifications = () => {
    ...
    useEffect(() => {
        ...
        if (!token || !authUser) {
            ...
            setNotifications([]);
            // toast.error("Authentication required to view notifications.");
            return;
        }
    ...
    const getNotificationIcon = (type: string) => {
        switch (type) {
            case "appointment_confirmed": case "appointment_canceled": case "appointment_pending": return Calendar;
            case "promotion": case "welcome": return Gift;
            case "reminder": return AlertCircle;
            case "review_request": return Star;
            case "system_update": return Settings;
            case "new_feature": return HelpCircle;
            case "account_security": return ShieldCheck;
            default: return Bell;
        }
    };
    ...
    const getNotificationColorClass = (type: string) => {
        switch (type) {
            ...
            case "system_update": case "new_feature": return "text-indigo-600";
    ...
    if (!authContextLoading && !authUser) { // Sprawdź authUser zamiast token, bo user może być potrzebny do wyświetlenia czegoś
        return (
            <div className="p-6 text-center">
                <p className="text-red-500">Could not authenticate user to view notifications.</p>
                                            ...
                                            <div className="flex items-start space-x-3 flex-1">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-xs text-gray-500 mt-1.5 sm:mt-2">
                                                        {isValid(new Date(notification.created_at)) ? formatDistanceToNow(new Date(notification.created_at), { addSuffix: true }) : "Invalid date"}
                                                    </p>
                                                </div>
                                            </div>
